...
import { SessionInfo } from 'src/auth/session-info.decoration';
import { GetSessionInfoDto } from 'src/auth/auth.dto';
import { BlockListService } from './block-list.service';

@Controller('block-list')
@UseGuards(AuthGuard) //получаем сессию
export class BlockListController {
  ...
  getList(
    ...
    return this.blockListService.getBlockListByUser(session.id, query);
  }
